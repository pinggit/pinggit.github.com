...
R2:
    ...
    set interfaces ge-1/2/1 unit 210 family inet filter input filter-block-ipv4
...
### troubleshooting pre-config
    ...
    1) 所有R上更正NHS policy ：external => route-type external
...
R2/3:
    ...
    set policy-options policy-statement exp-bgp-t-out term 1 from protocol bgp
...
1) R2/3/7/8 & R5/6 配置`family inet labeled-unicast rib inet.3`：
...
3) R2/3/5: 向P通告时，过滤V4V6各自的specific routes. （查看需求，是否需要对T同样做）  
...
* PE1-ASBR1，ASBR1-ASBR2，ASBR2-PE2:   
...
#### basic framework and vrf-imp/exp
...
#### VRF-CE policy
    ...
    set routing-instances GREEN protocols bgp export vrf-exp-bgp
    set routing-instances GREEN protocols ospf export vrf-exp-ospf
...
2) R2: 
    ...
    set protocol mpls explicit-null
...
### config LAG
    ...
    set interfaces ge-1/2/4 apply-groups-except interface-group
...
check and delete "explicit-null"
...
### config P/T block step1: all C/P/T facing R: import : tag routes
...
R4: need to build ipv4 bgp, need config RID

    
### config:6PE:PE-PE (solution4)

all R (except optional R8):

    #got all inet.3 already from rsvp/ldp, `ipv6-tunneling` will generate inet.6. 
...
R2/3/7/(8) & R5/6:
    ...
    set logical-systems r4 protocols bgp group to-rr family inet labeled-unicast rib inet.3     #<--for VPN
...
R5/6: 
    ...
    set logical-systems r5 policy-options policy-statement exp-bgp-adv-2378 term 1 from route-filter 10.200.1.2/32 exact
    ...
    set logical-systems r6 policy-options policy-statement exp-bgp-adv-2378 term 1 from route-filter 10.200.1.8/32 exact #<--MUST
...
R2,R3,R7 (R8 optional):

    #R2/3/7 learn some lo0 via BGP(not rsvp/ldp), "ipv6-tunnel" won't generate inet6.0; have to use rib-group
    #"manually" copy inet.3 into inet6.3
    set logical-systems r2 routing-options rib-groups inet.3-to-inet6.3 import-rib [ inet.3 inet6.3 ]
    set logical-systems r2 protocols bgp group to-rr family inet labeled-unicast rib-group inet.3-to-inet6.3
    
    set logical-systems r3 routing-options rib-groups inet.3-to-inet6.3 import-rib [ inet.3 inet6.3 ]
    set logical-systems r3 protocols bgp group to-rr family inet labeled-unicast rib-group inet.3-to-inet6.3

    set logical-systems r7 routing-options rib-groups inet.3-to-inet6.3 import-rib [ inet.3 inet6.3 ]
    set logical-systems r7 protocols bgp group to-rr family inet labeled-unicast rib-group inet.3-to-inet6.3
    
    set logical-systems r[[8]] routing-options rib-groups inet.3-to-inet6.3 import-rib [ inet.3 inet6.3 ]
    set logical-systems r8 protocols bgp group to-rr family inet labeled-unicast rib-group inet.3-to-inet6.3

### config: IPv6 routes aggregation

R1/2/3/4/7/8: adv IPv6 lo0

    set logical-systems r1 policy-options policy-statement exp-bgp-lo0-v6 term 1 from route-filter 2011:0310:1020::1/128 exact
...
R5-R6: adv IPv6 lo0 & aggr.ed IPv4&IPv6 routes
    ...
    set logical-systems r5 policy-options policy-statement exp-bgp-lo0-v6 term 1 from route-filter 2011:0310:1020::5/128 exact
    ...
    set logical-systems r5 policy-options policy-statement exp-bgp-lo0-v6 term 3 from route-filter 10.200.0.0/16 exact
    ...
    set logical-systems r6 policy-options policy-statement exp-bgp-lo0-v6 term 1 from route-filter 2011:0310:1020::6/128 exact
...
R2/3/5: change existing policy P:p-out reject pecific
...
### config:RTBH:imp:NH discard

NOTE: 
    

    set logical-systems r2 policy-options policy-statement imp-bgp-rtbh term 1 from protocol bgp 
    community rtbh 
    route-filter 0.0.0.0/0 prefix-length-range /32-/32
    set logical-systems r2 policy-options policy-statement imp-bgp-rtbh term 1 then next-hop discard accept
    set logical-systems r2 policy-options policy-statement imp-bgp-rtbh term 1 then accept      #<------MUST
    ...
    set logical-systems r2 policy-options policy-statement imp-bgp-rtbh term 2 then accept      #<------MUST
    set logical-systems r2 policy-options policy-statement imp-bgp-rtbh term 3 from protocol bgp community rtbh
    ...
      
    set logical-systems r3 policy-options policy-statement imp-bgp-rtbh term 1 from protocol bgp community rtbh route-filter 0.0.0.0/0 prefix-length-range /32-/32
    set logical-systems r3 policy-options policy-statement imp-bgp-rtbh term 1 then next-hop discard
    set logical-systems r3 policy-options policy-statement imp-bgp-rtbh term 1 then accept      #<------MUST
    set logical-systems r3 policy-options policy-statement imp-bgp-rtbh term 2 from protocol bgp community rtbh route-filter ::/0 prefix-length-range /128-/128
    set logical-systems r3 policy-options policy-statement imp-bgp-rtbh term 2 then next-hop discard
    set logical-systems r3 policy-options policy-statement imp-bgp-rtbh term 2 then accept      #<------MUST
    set logical-systems r3 policy-options policy-statement imp-bgp-rtbh term 3 from protocol bgp community rtbh
    ...
    set logical-systems r5 policy-options policy-statement imp-bgp-rtbh term 1 from protocol bgp community rtbh route-filter 0.0.0.0/0 prefix-length-range /32-/32
    ...
    set logical-systems r5 policy-options policy-statement imp-bgp-rtbh term 2 from protocol bgp community rtbh route-filter ::/0 prefix-length-range /128-/128
    ...
    set logical-systems r5 policy-options policy-statement imp-bgp-rtbh term 3 from protocol bgp community rtbh
    ...
    set logical-systems r5 protocols bgp group to-r6 import imp-bgp-rtbh

### config:RTBH:exp:fix NH for C routes

    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 1 from protocol bgp
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 1 from neighbor 10.200.1.1
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 1 from community rtbh
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 1 from route-filter 0.0.0.0/0 prefix-length-range /32-/32
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 1 then next-hop 10.200.1.1
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 2 from protocol bgp
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 2 from neighbor 10.200.1.1
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 2 from community rtbh
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 2 from route-filter ::0/0 prefix-length-range /128-/128
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 2 then next-hop ::ffff:10.200.1.1

    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 3 from protocol bgp
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 3 from neighbor 10.200.1.4
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 3 from community rtbh
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 3 from route-filter 0.0.0.0/0 prefix-length-range /32-/32
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 3 then next-hop 10.200.1.4
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 4 from protocol bgp
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 4 from neighbor 10.200.1.4
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 4 from community rtbh
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 4 from route-filter ::0/0 prefix-length-range /128-/128
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 4 then next-hop ::ffff:10.200.1.4

    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 5 from protocol bgp
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 5 from neighbor 10.200.1.7
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 5 from community rtbh
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 5 from route-filter 0.0.0.0/0 prefix-length-range /32-/32
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 5 then next-hop 10.200.1.7
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 6 from protocol bgp
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 6 from neighbor 10.200.1.7
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 6 from community rtbh
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 6 from route-filter ::0/0 prefix-length-range /128-/128
    set logical-systems r5 policy-options policy-statement exp-bgp-fix-nh-rtbh term 6 then next-hop ::ffff:10.200.1.7

    set logical-systems r5 protocols bgp group rr export exp-bgp-fix-nh-rtbh         
    set logical-systems r5 protocols bgp group to-r6 export exp-bgp-fix-nh-rtbh 
...
R5: 
    ...
    set logical-systems r5 protocols bgp group to-asbr neighbor 100.5.22.2 peer-as 2
    ...
    set logical-systems r5 policy-options policy-statement exp-bgp-adv-1 term 3 then reject
    
    set logical-systems r5 protocols bgp group to-asbr neighbor 100.5.22.2 export exp-bgp-adv-1

R1:

    #config MH-MP-EBGP to remote PE; 
    #to ASBR enable inet unicast+labeled-unicast; 
    #to remote PE also enable inet-vpn
    set logical-systems r1 protocols bgp group to-rr family inet unicast
    set logical-systems r1 protocols bgp group to-rr family inet labeled-unicast rib inet.3
    set logical-systems r1 protocols bgp group to-pe multihop
    set logical-systems r1 protocols bgp group to-pe neighbor 10.10.10.10 family inet-vpn unicast
...
### config: R1 VRF to local CE

R1 VRF:
    
    set logical-systems r1 routing-instances PINK instance-type vrf
...
### prepare:borrow ae ports to be used as vpls ports

    deactivate interfaces ge-1/2/3 gigether-options 802.3ad    
...
R1/7/8: vpls inteface

    set logical-systems r1 interfaces ge-1/2/3 unit 100 vlan-id 100 encapsulation vlan-vpls apply-groups-except interface
...
R1/7/8/2/3 BGP family L2vpn signaling:
...
NOTE: 
* must fully verify . anything works indicate sth wrong in previous parts
...
#### R1:
    ...
    set logical-systems r1 routing-instances GREEN instance-type vrf
...
#### R4:
    ...
    set logical-systems r4 policy-options community community ce2 members target:65000:2;
    set logical-systems r4 policy-options community community ce3 members target:65000:3;
    
    set logical-systems r4 routing-instances GREEN instance-type vrf
...
#### R8:
    ...
    set logical-systems r8 routing-instances GREEN instance-type vrf
    ...
    set logical-systems r8 routing-instances GREEN protocols bgp group to-ce neighbor 200.6.84.2 peer-as 65000
    set logical-systems r8 routing-instances GREEN protocols bgp group to-ce neighbor 200.6.84.2 as-override
    set logical-systems r8 routing-instances GREEN protocols bgp group to-ce neighbor 200.6.85.2 peer-as 65000
    set logical-systems r8 routing-instances GREEN protocols bgp group to-ce neighbor 200.6.85.2 as-override

### config: VPN policy

    set logical-systems r1 policy-options policy-statement vrf-exp-ospfterm 1 from protocol bgp
    set logical-systems r1 policy-options policy-statement vrf-exp-ospf term 1 from community GREEN    #<------optional
    set logical-systems r1 policy-options policy-statement vrf-exp-ospf term 1 then accept
    set logical-systems r1 routing-instances GREEN protocols ospf export vrf-exp-ospf

    set logical-systems r4 policy-options policy-statement vrf-exp-ospf term 1 from protocol bgp
    set logical-systems r4 policy-options policy-statement vrf-exp-ospf term 1 from community GREEN
    set logical-systems r4 policy-options policy-statement vrf-exp-ospf term 1 then accept     
    set logical-systems r4 policy-options policy-statement vrf-exp-ospf term 2 then reject      #<------optional
    
    set logical-systems r4 policy-options policy-statement vrf-exp-bgp term 1 from protocol ospf
    set logical-systems r4 policy-options policy-statement vrf-exp-bgp term 1 then accept
    set logical-systems r4 policy-options policy-statement vrf-exp-bgp term 2 from protocol direct
    set logical-systems r4 policy-options policy-statement vrf-exp-bgp term 2 from route-filter 200.6.42.0/24 exact
    set logical-systems r4 policy-options policy-statement vrf-exp-bgp term 2 then accept
    
    set logical-systems r4 routing-instances GREEN protocols bgp export vrf-exp-bgp
    set logical-systems r4 routing-instances GREEN protocols ospf export vrf-exp-ospf


### config: shamlink

    set logical-systems r1 routing-instances GREEN protocols ospf sham-link local 1.1.1.1
...
### config: VPN InetAccess 1

solution 1: inet.0 static + vrf aggr def + inet.0 rib-group
...
4. R1-vrf=>Inet: R1-global: leak full Internet table into R1-vrf, upward done.
    ...
    #step 1/2 
    set logical-systems r1 routing-options static route 20.20/16 next-table GREEN.inet.0 
    ...
    set logical-systems r1 policy-options policy-statement vrf-exp term 3 then community add GREEN      #<------don't forget
    ...
    set logical-systems r1 protocols bgp group to-c3 family inet unicast rib-group imp-inet.0-to-vrf    #<------don't forget

### config: VPN InetAccess 2: inet.0 static+2rib-group

    #R1:VRF -> Internet: R1 config static def pointing to R2/3; then rib-group it to vrf
...
### config vpn LSP map
    ...
    set logical-systems r1 policy-options policy-statement exp-fwd-lsp-map term 1 from protocol bgp
...
* Task 1: 3 points You have agreed to permit special queuing for traffic from
customer C3 to peer P3. Ensure that your router classifies traffic from C3
according to the markings in this section’s exhibit.

* Task 2: 6 points Police the traffic according to the information in this
section’s exhibit. Ensure that these forwarding classes are preserved to both
peering locations of P3, even in the case of core link failures.

### config COS

[from sp3]

    1,Provide COS service to the traffic flow from C3 to P3. 
...
R1:
    ...
    #(router will then rewrite MPLS EXP per fwd-class => exp rewrite rule) 
    #apply policers under firewall filter
    set logical-systems r1 firewall filter J-Classifier term 1 from destination-address 211/8 precedence 7
    ...
    set logical-systems r1 firewall filter J-Classifier term 1 from destination-address 211/8 precedence 6
    ...
    set logical-systems r1 firewall filter J-Classifier term 3 from destination-address 211/8   #<-check the exam requirement
...
### Device Task
1. bind GE interface between R3 and R6,if any physical link down, the AE
   interface must go down either.
2. Enable Graceful restart on each core devices R1-R8
3. R2 and DC are running ospf ,achieve sub-second convergence between them.(
   enable BFD on ospf )
   
NOTE: check the group ‘interface”’s configuration, make sure enable” mpls/iso/inet6” on the AE interface also.

1. Task 1: 3 points Interfaces facing T routers must discard and log to syslog
   ...
   which are the ranges used internally.
   
1. Task 2: 3 points Ensure that all configuration changes on R1 are backed up
   to a server when you commit the changes. The server’s details are shown in
   this section's exhibit.
 
### verify:transfer-on-commit
...
### TIPS: firewall filter: don't forget the last term of `then accept`
...
### QUESTION(resolved): IMPORTANT: scp doesn't work
...
this works if scp server is another junos router, not work for linux, don't know.

### QUESTION(resolved): about `apply-group-except`

in this lab, under which condition this is required?

A: like in vpls part, need this to exclude other family and only keep vpls
   (otherwise commit check fail)
...
1. check isis adjacency, only level 2 neighbor will be allowed
2. R2 and DC are running ospf, on R2, 
   * reject prefix: 172.16.0.0/24~172.16.7.0/24 which learned from DC ,and 
   * redistribute Ospf routes into ISIS with only 2 routes 
   * R2 will send 10.200/16 to DC and 
   * send an default route only when R2 has active BGP routes. 
3. disable ipv6 routing on ISIS
4. adjust the GE’s metric to 5 


1. Task 1: 6 points Ensure that all internal IPv4 networks are reachable and
   ...
   (make basic isis l2 work)
2. Task 2: 4 points The routes in this section’s exhibit are being received
   ...
   (R2:isis export: summarize with 2 and adv)
3. Task 3: 2 points Announce the 10.200.0.0/16 prefix to the data center. Also
   announce a default route to the data center only when BGP routes are present
   on R2.
   (R2:ospf exp: adv only summariazed)
   (R2:ospf exp: adv conditional def)
4. Task 4: 1 point Ensure that IS-IS link metrics are automatically calculated
   so that a 1-Gbps interface has a metric of 5.
   (isis:ref-bw)
5. Task 5: 1 point Ensure that no IPv6 addresses can appear as IS-IS routes.
   (isis: no-ipv6)
...
which is right in this case? I think `generate route`

manual:

Generated routes are used as the route of last resort. A packet is forwarded to
...
- A `static route` is the most obvious. You need to be able to reach a certain
  ...
  -preferred- than that of any dynamic RP).
- An `aggregate route` is a route you define but which is not used for forwarding
  ...
  prefers aggregates.
- A `generated route` is technically an aggregate route but which can be used to
  ...
  upstream routes - ie: provide connectivity if certain upstream routes exist.
...
1. check each router’s LSP, make sure ALL-LSP are up .
2. you can’t enable LDP on R2/R3, can’t enable RSVP on R7/R8, find a way to
   exchange label between R1/R4and R7/R8. =>  (LDPoRSVP)
3. the LSP r2-r5 must transit on two paths, the primary path transit r4 and r6,
   the secondary path transit r3, both paths must be UP simultaneously. =>
   (r2-r5 primary/secondary standby)
4. set up the secondary path for lsp r4-r3,2nd path must remain up and the
   reservation bandwidth is 600m, 2 paths’s first hop must be R2. =>
   (R4-r3 primary/secondary standby)
5. LSP r3-r5 can’t pass through the link between r3-r5, achieve this without
   using ERO. => (admin-group)
6. LSP r1-r6 shall be preempted by other LSP => (preemption)
7. IPV6’s traffic must be tunneled in MPLS => (ipv6-tunnel)
8. R5/R6 shall not has label “0” => (R5/6: explicit-null)
9. configure LSP r3-r4 , the reservation bandwidth will be 600m and ensure the
   traffics will not exceed 600m either, r3-r4 will not use r3-r2’s link , ERO
   was forbidden => (r3-r4 bw resv/policer; admin-group)
10. on R6, configure the load balance based on the ip layer 3 AND MPLS
    first-label information (R6: LB)

A:

1. Task 1: 2 points Ensure that R2 and R3 do not use LDP for performing MPLS
   functions.
2. Task 2: 3 points Ensure that R7 and R8 do not use RSVP for signaling LSPs
...
3. Task 3: 3 points LSPs have been preconfigured, as shown in this section’s
   ...
   section’s exhibit.
4. Task 4: 2 points Using the existing admin groups, ensure that the r3-to-r4
   LSP avoids the R2–R3 link.
5. Task 5: 3 points Configure the r3-to-r4 LSP to reserve 600Mbps of
   ...
   Ensure that all traffic in excess of the reserved bandwidth is dropped.
6. Task 6: 3 points On R6, ensure that all MPLS packets transiting the R3-R6
   ...
   3 IP packet header are used by the load-balancing algorithm.
7. Task 7: 3 points Configure the network so that all IPv6 traffic will be
   tunneled through MPLS LSPs.
8. Task 8: 1 point Ensure that R5 does not allocate a label value of 0 to any
   IBGP, RSVP or LDP neighbors.

B:

1. Task 1: 2 points Ensure that R2 and R3 do not use LDP for performing MPLS
   functions. => (R2/3 RSVP only)
   
1. Task 2: 3 points Ensure that R7 and R8 do not use RSVP for signaling LSPs in
   ...
   (R1 LDPoRSVP to R7/8, no single link failure)
   
1. Task 3: 3 points LSPs have been preconfigured, as shown in this section’s
   exhibit, and are currently operational in the network. (Functionality of
   ...
   specified in this section’s exhibit. => (r2-to-r5: primary/secondary with ERO)
   
1. Task 4: 2 points Using the existing admin groups, ensure that the r2-to-r5
   LSP avoids the R2–R3 link. => (r2-to-r5: admin-group)
   
1. Task 5: 3 points Configure the r3-to-r4 LSP to reserve 600Mbps of bandwidth
   across the network. Ensure this LSP can be preempted by all other
   RSVP-signaled LSPs in the network. => (r3-to-r4: bw/preemption)
   
1. Task 6: 3 points Use a bypass LSP to improve convergence time for the
   r2-to-r5 LSP in the event of an R6-R5 link failure. =>
   (r2-to-r5:link-protection)
   
1. Task 7: 3 points Configure the network so that all IPv6 traffic will be
   tunneled through MPLS LSPs. => (ipv6-tunneling)
   
1. Task 8: 1 point Ensure that R5 does not allocate a label value of 0 to any
   IBGP, RSVP or LDP neighbors. => (r5: explicit-null)
    ...
    r5-to-r2 should transit R3 and R6.

### verify:load balance

    run request pfe execute command "show jnh lb" target tfeb0
...
### QUESTION: link color need on both side?

    seems no need to config on both side?
...
### TIP: RSVP resv style: FF vs. SE(adaptive)

default is Fixed Filter (FF)

keyword: "adaptive": change that to a Shared Explicit (SE) style;

SE style reservations allow resources to be shared among multiple LSPs that
share a common Session Object parameter.

### QUESTION: config LB in 12.3

http://www.juniper.net/techpubs/en_US/junos12.3/information-products/pathway-pages/config-guide-policy/config-guide-policy.pdf

it looks, these are supported in MX80 before 12.3

      
`enhanced-hash-key` after 12.3
http://www.juniper.net/techpubs/en_US/junos12.1/topics/reference/configuration-statement/enhanced-hash-key-edit-forwarding-options.html
...
1. R5/R6 will be global RR, R1,R2,R3,R4,R7,R8 will be GRR’s clients 
2. R2/R3 will be VPN RR, R1,R4,R7,R8 will be VRR’s clients
3. R5/R6 , R2/R3 will set up normal IBGP peer relationship.
4. Migrate all the core router from AS:65513 to AS:4012345678,but C3 still peer
   with 65513, set up the bgp peer relationship on R1 and hide the 65513 from
   the Core.
   
5. all the C devices will receive C/P/T routes, P devices will receive P/C
   routes, T devices will receive T/C routes.
   => block P with T
   
6. You can’t change the interface’s address setting on R1/R4/R7, only one NLRI
    ...
    * C3 and R1 , C5 and R4 only running IPV6
    * C4 and R7 running IPV4/IPV6 Dual Stack
    * C3/C4 can reach every IPV4/V6 address in the testing topology
    
    => 6PE

7. Advertising 10.200/16 and 2011:0310::/32 to C/P/T devices, P device can only
...
NOTE: Your network is transitioning to a 4-byte AS number, which is shown on
...
and addresses you may use to test reachability.

1. Task 1: 3 points Ensure that R5 and R6 are route reflectors for only IPv4
...
2. Task 2: 7 points Ensure that all BGP sessions are up. Ensure that C3 and C4
...
3. Task 3: 3 points Ensure that the network uses the AS number shown on the
   ...
   extra ASs appear in the AS path. => `local-as 65513 private`
   
4. Task 4: 5 points Ensure that you send to each P router only the
   ...
   more specific than /20 or IPv6 prefixes more specific than /48.
   
5. Task 5: 10 points C4 and C5 are both sending IPv6 aggregates as shown in
   ...
   => adv def is now allowed

B:

NOTE: Your network is transitioning to a 4-byte AS number, which is shown on
the diagram. Most of your EBGP peers are already peering to the 4-byte AS
number; you need to adjust your network to accommodate these peerings.

See this section’s exhibit for information about the aggregates each AS will
send, and addresses you may use to test reachability.

1. Task 1: 4 points Ensure that R5 and R6 are route reflectors for only IPv4
   and IPv6 routes. Ensure that R2 and R3 are route reflectors for only VPN
   routes. The necessary sessions have been preconfigured for you. You may
   modify the attributes of these sessions, but you may not establish any new
   IBGP sessions.
2. Task 2: 9 points Ensure that all BGP sessions are up. Ensure that C3 and C4
   can reach all IPv4 destinations. You may not advertise a default route to C3
   or C4. Ensure that all IPv4 routes you receive from C3 and C4 are advertised
   to all T routers. Ensure that traffic follows the optimal path within your
   AS.
3. Task 3: 3 points Ensure that the network uses the AS number shown on the
   ...
   This customer’s router does not support 4-byte ASs, but the session was
   previously established with R1 using a local AS of 65513. Ensure that no
   extra ASs appear in the AS path.
4. Task 4: 5 points C3 and C5 are both sending IPv6 aggregates as shown in this
   section’s exhibit. Ensure that these aggregates are advertised to all
   transit providers. Ensure that C3 and C5 can reach all IPv6 destinations.
   You may not advertise a default route to C3 or C5. Note: The session to C5
   is not preconfigured.
   
5. Task 5: 7 points You have agreed to help your customers respond to
   denial-of-service (DoS) attacks aimed at one of their hosts. You will do
   this by blocking traffic destined for one of their hosts when the customer
   sends you a specific BGP announcement for that host. If a C router sends a
   /32 IPv4 or /128 IPv6 route with the community target:4012345678:2000 or
   23456:2000, ensure that traffic with that destination address is not allowed
   to enter your network from transit providers or peers. Strip any of these
   communities if they exist on non-host routes or if they are sent from T
   routers or P routers.
   
   => if from C && host route && communties 
            next-hop discard
      elseif (from any) non-host route
            delete communities
      elseif from P/T && community      #<------?
            delete communities 
      else
            do nothing
...
> C 可以达到 C,P,T。P 之间可以互访，T 之间可以互访，P 与 T 之间不能互
...
>       => block P/T
>
> 6,Advertising 10.200/16 and 2011:0310::/32 to C/P/T devices, P device can only
> receive IPV4 prefix less and equal than /20, IPV6 prefix less and equal than
> /48
>       => to C/T/P:
>               adv IPv4/IPv6 agg routes
>       => to P:
>               block longer IPv4/IPv6 prefix to P


### pre-config:basic BGP RR VPN

#### inet/inet6 RR: R5/R6

    set logical-systems r1 protocols bgp group to-rr type internal
...
### verify: AS migration/hide

with `local-as private`
...
R1 -> C3:
    ...
    lab@MX80-NGGWR-02# run show route advertising-protocol bgp 100.1.33.2 logical-system r1 
...
* running on top of LSP (ldp or rsvp based)     
* Juniper PE core facing i/f `must` enable ipv6 => troubleshooting tip.
  (seems only if use label 2). otherwise forwarding will have issue.
...
3. mix 1/2
4. RRs (R5/6) BGP: adv R2/3/7/8 , so all R has each other's lo0 in BGP
   all Rs(R2/3/5/6/7/8) BGP: 
       inet labeld-unicast rib inet.3 => put learned labeled lo0 into inet.3
   R2/3,R7/8 BGP:
       inet labeld-unicast rib-group 6pe => copy inet.3 into inet6.3
 
notes:
R8 is not involved here (no C/P/T router attached, only for MPLS/VPN)

#### routing:

* CE2 adv IPv6 routes to PE2, via IPv6 protocols (BGP, ISIS, OSPFv3, RIPng, etc)
* PE2 get CE2 IPv6 routes, with NH as CE2 IPv6 address
* `family inet6 labeled-unicast` => 
   - PE2 adv labeled(`explicit-null` => label 2) IPv6 routes to PE1, via MP-BGP
* `protocol mpls ipv6-tunneling` => 
   - PE1 check inet3.0 (table of map: IPv4 <=> LSP )
   - PE1 convert all IPv4 in inet.3 to IPv6, in a form of "IPv4 mapped IPv6 address"
   - PE1 put all IPv4-m-IPv6 into inet6.3 (a table of map: IPv6 <=> LSP)
   - PE1 BGP now can: 
                  
                      (auto-convert based on IPv4 NH)
            resolve IPv6 routes ===> NH IPv4-m-IPv6
                            (inet6.3)
            resolve IPv4-m-IPv6 ===> LSP
        
* PE1 adv IPv6 routes to CE1, NH to PE1 IPv6
...
2. PE CE都使用了IPv4-c-IPv6 地址，不允许更改（为IPv4-m-IPv6)
 
因为1, 所以只能用V4的BGP session,配置family inet6 传递V6 路由信息
但（V6规范或者JUNOS规则要求），V6路由信息只能用以下V6 地址做NH:
* 明确配置的远端V6 lo0
* 系统根据V4 lo0 生成的IPv4-m-IPv6 地址

根据（JUNOS) EBGP规则，从single-hop的EBGP
...
解决方法是配置“accept-remote-nexthop”

现在路由是显现了，但缺省规则也只是以V4-m-V6做NH,但lab中本地配置的是V4-c-V6,因此需要改变NH

#### PE to PE
                                                      
1. PE之间建立bgp IPv4 session, 彼此双方PE的inet6
   路由(此处称为6PE路由，相对于MPLSVPN中的vpn路由)以labeled inet6 NLRI形式传递,
   (使用explicit-null 则用 label 2 -- 这是junos 6PE “标准”label);
   决定这个的配置是`family inet6 labeled-unicast explicit-null`
2. 因为有了可携带inet6 NLRI的BGP session,中间怎么乱都没所谓，路由信息都可以通过去
3. 因为是inet6 的NLRI ，所以NH就自动使用v4-m-v6 (疑为IPv6规范)
 
        Source: 10.200.1.5
        Protocol next hop: ::ffff:10.200.1.7
                
4. 现在需要resolved 这个形式为v4-m-v6 的NH(解析为LSP),
   JUNOS的实现规则，需要查找inet6.3
5. 一切复杂性开始了，都是关于如何生成inet6.3. lab中有两个场景。

#### 场景1:

1. R1有点特殊。它恰好已从RSVP(R2/3/4), 和LDPoRSVP(R5/6/7/8)学到了所有的IPv4 lo0.
2. junos缺省规则：为ldp/rsvp路由缺省生成inet.3
3. R1的配置"mpls ipv6-tunnel", 使得inet.6 可以根据inet.3做自动转换而生成，
   “转换”基本就是把prefix 从IPv4 换为v4-m-v6
4. R1 问题解决了。

#### 场景2:
1. 复杂问题在于R2/3/7/8. 因为R5/6分割了ldp/rsvp的连通性。所以R2/3
   和R7/8之间互相通过LDP或者RSVP学到对方的IPv4 lo0, 也就是说，inet.3中缺这些东西
2. 解决的方法，按照solution 4, 就用连贯的bgp 取代不连贯的ldp/rsvp，
   充当一个新的label 分配协议。也就是`inet labeled-unicast`;
3. 但如果不配置`rib inet.3`的话， 这个新的inet labeled-unicast和原来的inet
   ...
   中，这是JUNOS bgp的缺省行为
4. 解决方法是使用`rib inet.3`配置，`inet labeled-unicast rib inet.3`， 这个配置
   干了两个事情: 
   现在inet labeled unicast不与inet unicast冲突: 
   inet.0没有影响
   RR上通告的labeled-unicast 的v4 lo0 则另外放在inet.3，这样就有了v4 NH 的inet.3
5. 此时细看r1的inet.3， 必定既有ldp/rsvp的，也有bgp来的；
   但根据preference，选择ldp/rsvp的。只有新生成的r7/8的lo0
   用了bgp的。这些细节都不重要了。
6. 只是这时候仍然没有inet.6， 但根据前面的原则， ipv6的NLRI硬性要求ipv4-m-v6
   为NH，要解析的也是v6 NH. 也就是说必须要有inet.6， 但JUNOS规则，“mpls
   ipv6-tunneling” ， 不会对BGP生成的inet.3 生成inet6.3 ， 因此手工用rib-group
   生成。

#### 规范规则
这个过程中涉及到不少潜在的规则或者规范，（也是考试设置的环境特意要考察的知识点）：
1. `family inet6 labeled-unicast explicit-null` => 为6PE路由分配“VPN” label 2
1. inet6的BGP prefix，要求NH必须为V6, 因此系统自动根据IPv4 lo0生成IPv4-m-IPv6 作为NH
1. LDP/RSVP学习到的lo0 自动放置在inet.3 ，BGP不会。除非配置`inet labeled-unicast rib inet.3`
1. `mpls ipv6-tunnel`为LDP/RSVP的inet.3生成inet6.3, 不会为BGP的inet.3生成。使用rib-group 手工解决
...
#### without `accept-remote-nexthop` and nh changing policy

not accept any route at all, not even in hidden

    test@MX80-NGGWR-01# run show bgp summary logical-system r1 
    ......
...
#### fix incoming NH with import policy
    
rib-in still unchanged:
...
routes in `rib` is changed, with new NH:

    (IPv4-mapped-IPv6)          (IPv4-compatible-IPv6)
...
### verify:6PE label path:C3-C4

R1->R2: label 299904

    lab@MX80-NGGWR-02# run show route forwarding-table matching 4444:4444:4444::/48 | find r1 
    Logical system: r1
...
R5 rib-in (before imp policy): 
initial C3 routes from R1

* non-host routes also with 2 communities, need to delete


    test@MX80-NGGWR-01# run show route logical-system r2 community-name rtbh              

    inet.0: 84 destinations, 102 routes (83 active, 0 holddown, 3 hidden)
    Restart Complete
    + = Active Route, - = Last Active, * = Both

    3.3.3.0/24          [BGP/170] 12:45:23, localpref 100, from 10.200.1.6
                          AS path: 300 I
                        > to 100.0.12.1 via ge-1/2/2.102, label-switched-path r2-to-r1
    33.33.0.0/16        [BGP/170] 12:45:23, localpref 100, from 10.200.1.6
                          AS path: 300 I
                        > to 100.0.12.1 via ge-1/2/2.102, label-switched-path r2-to-r1
    33.33.33.33/32     *[BGP/170] 00:01:42, localpref 100, from 10.200.1.5
                          AS path: 300 I
                        > to 100.0.25.2 via ge-1/2/1.205, label-switched-path r2-to-r5
                        [BGP/170] 00:01:41, localpref 100, from 10.200.1.6
                          AS path: 300 I
                        > to 100.0.25.2 via ge-1/2/1.205, label-switched-path r2-to-r5
    ...
    inet6.0: 47 destinations, 71 routes (47 active, 0 holddown, 0 hidden)
    Restart Complete
    + = Active Route, - = Last Active, * = Both

    ::3.3.3.0/120       [BGP/170] 12:45:23, localpref 100, from 10.200.1.6
                          AS path: 300 I
                        > to 100.0.12.1 via ge-1/2/2.102, label-switched-path r2-to-r1
    ::33.33.33.33/128  *[BGP/170] 00:01:42, localpref 100, from 10.200.1.5
                          AS path: 300 I
                        > to 100.0.25.2 via ge-1/2/1.205, label-switched-path r2-to-r5
                        [BGP/170] 12:45:23, localpref 100, from 10.200.1.6
                          AS path: 300 I
                        > to 100.0.12.1 via ge-1/2/2.102, label-switched-path r2-to-r1
    3333:3333:3333::/48 [BGP/170] 12:45:23, localpref 100, from 10.200.1.6
                          AS path: 300 I
                        > to 100.0.12.1 via ge-1/2/2.102, label-switched-path r2-to-r1

    inet6.3: 7 destinations, 11 routes (7 active, 0 holddown, 0 hidden)
    Restart Complete
    [edit]
    lab@MX80-NGGWR-02# run show route protocol bgp logical-system r5 receive-protocol bgp 10.200.1.1 table inet.0 detail  

    inet.0: 50 destinations, 64 routes (50 active, 0 holddown, 0 hidden)
    Restart Complete
    ...
    * 20.20.0.0/16 (2 entries, 1 announced)
         Accepted
         Nexthop: 10.200.1.1
         Localpref: 100
         AS path: I
         Aggregator: 4012345678 10.200.1.1

    * 33.33.0.0/16 (2 entries, 1 announced)
         Accepted
         Nexthop: 10.200.1.1    #<------
         Localpref: 100
         AS path: 300 I
...
R5 rib(after imp policy)

* the non-host routes: removed the 2 communities
* the non-host routes: don't change next-hop
* host routes: keep the 2 communities
* host routes: next-hop set to discard
    ...
    33.33.33.33/32     *[BGP/170] 00:00:41, localpref 100, from 10.200.1.5
                          AS path: 300 I
                          Discard
                        [BGP/170] 00:00:40, localpref 100, from 10.200.1.6
                          AS path: 300 I
                          Discard
    ...
    ::33.33.33.33/128  *[BGP/170] 00:00:41, localpref 100, from 10.200.1.5
                          AS path: 300 I
                          Discard
                        [BGP/170] 12:44:22, localpref 100, from 10.200.1.6
                          AS path: 300 I
                          Discard
    ...
    [edit]
    lab@MX80-NGGWR-02# run show route protocol bgp logical-system r5 3.3.3.0/24 detail  
    ...
    [edit]
    lab@MX80-NGGWR-02# run show route protocol bgp logical-system r5 33.33.33.33/32 detail  
...
### TIP: R2 policy term4 is a must

    lab@MX80-NGGWR-02# run show route logical-system r2 10.200/16 
...
### NOTE: suppress specific route , including C or not

currently suppress all specific routes.
if the target is meant to suppress specific routes from core, but not to
specific routes from C, then insert another term before term 2, 

    from community c-route
    then accept
...
R1-R8 has : aggr. V4 & V6 routes, all IPv6 lo0
...
R2 to P:  aggr V4&V6, no too specific routes
...
### QUESTION(resolved): bgp flaps duing config
...
normal, capability re-negotiation
...
### config solution1(old):

`inet (uncast+labeled-unicast rib inet.3)`
...
#### vpls forwarding-table (no traffic)
...
#### vpls forwarding table (with traffic)
...
#### bridge bridge domain (per-instance)
...
### TIP: vpls link redundency issue

by default system will disable one link. if the test ping happens to arrive
from the disabled link, ping won't succeed 
...
### 2 commit errors
    ...
    [edit logical-systems r1 interfaces ge-1/2/3]
      'unit 100'
         Link encapsulation type is not valid for device type
    error: configuration check-out failed

config encap flexible-ethernet-service resolve it

    lab@MX80-NGGWR-02# set interfaces ge-1/2/3 encapsulation flexible-ethernet-services  
...
1. Task 1: 5 points Create a Layer 3 VPN called GREEN, including the following
   sites: GREEN-CE1, GREEN-CE2, GREEN-CE3, GREEN-CE4 and GREEN-CE5. CE3, CE4
   and CE5 will peer using BGP. CE1 and CE2 are using OSPF Area 0. All CE
   routers must receive the routes announced from all other CEs.
   => basic VPN
   
2. Task 2: 5 points GREEN-CE1 and GREEN-CE2 share a backdoor connection. Ensure
   that CE1 and CE2 prefer to send traffic through the Layer 3 VPN. See this
   section's exhibit for additional details.
   => VPN sham link
   
3. Task 3: 3 point For the GREEN VPN, ensure that all CE routers can ping
   directly connected hosts on remote PE-CE links.
   => basic VPN adv direct link
   
4. Task 4: 5 points Ensure that the GREEN VPN has Internet access through the
   R1 router. You may configure 1 static route in inet.0 to accomplish this
   task.
   => VPN Internet access. 1 static in inet.0

5. Task 5: 5 points On R1, ensure that GREEN VPN traffic to CE2 uses the
   r1-to-r4-blue LSP.  On R1, ensure that GREEN VPN traffic to CE3 uses the
   r1-to-r4-green LSP.
   => VPN LSP map


### v0
1. VPN1,VPN2 running ospf with PE, VPN3,VPN4,VPN5 running BGP with PE, AS
   number: 65000
2. Each PE can only have one instance defined, and the name is:green
3. VPN1 and VPN2 has a backdoor ospf link, do not use this link until the MPLS
   VPN broke down.
4. R1 has two LSP named blue and green, for VPN2 , use LSP green, for VPN3, use
   LSP BLUE
5. Each VPN must have the access to others, including the link addresses.
6. VPN must have internet access, and 
    * the internet gateway is R1, 
    * you can’t define any static route in VRF instance, 
    * you can only define 1 static route on PE，
    * rib-group not allowed under VRF.

    
### verify bgp/ospf connections

core MP-BGP: from RR: R2/3: all neighbor established, with correct family
...
VRF-CE:
    ...
    lab@MX80-NGGWR-02# run show ospf neighbor logical-system r1 instance GREEN 
...
### verify solution1 data plane: e2e ping/traceroute

    run ping 20.20.1.22 logical-system r0 routing-instance vpn1 source 20.20.1.11 count 3 rapid
...
### verify2: shamlink
    ...
    lab@MX80-NGGWR-02# run show route logical-system r0 table vpn1.inet.0 protocol ospf    
...
### verify solution 1 e2e ping/trace

    lab@MX80-NGGWR-02# run ping 10.200.1.8 logical-system r0 routing-instance vpn1 source 20.20.1.11 rapid              
...
### verify : solution 2 ping/traceroute

* ping to C3 doesn't work, all other OK
* traceroute doesn't work
    ...
    --- 10.200.1.8 ping statistics ---
    5 packets transmitted, 5 packets received, 0% packet loss
    round-trip min/avg/max/stddev = 0.546/0.619/0.863/0.122 ms
    ...
    [edit]
    lab@MX80-NGGWR-02# run ping 44.44.1.1 logical-system r0 routing-instance vpn1 source 20.20.1.11  
    ...
    2 packets transmitted, 2 packets received, 0% packet loss
    round-trip min/avg/max/stddev = 0.634/0.692/0.750/0.058 ms
    
    [edit]
    lab@MX80-NGGWR-02# run traceroute 44.44.1.1 logical-system r0 routing-instance vpn1 source 20.20.1.11  
...
* this only change where to "lookup" , not to change/copy any routing tables
  themselves.
...
### solution 1 old notes

    #R1 global add static route summarizing all vpn routes, pointing into vrf
...
#### remove solution1
    ...
    #then adv to all internet via bgp
    delete logical-systems r1 policy-options policy-statement exp-bgp-adv-vpn
    
    delete logical-systems r1 protocols bgp group to-rr export exp-bgp-adv-vpn
    delete logical-systems r1 protocols bgp group to-c3 export exp-bgp-adv-vpn

    #R1 vrf adv a default route to CE1 
    delete logical-systems r1 routing-instances GREEN routing-options aggregate route 0.0.0.0/0
    delete logical-systems r1 policy-options policy-statement vrf-exp-ospf-def
    delete logical-systems r1 routing-instances GREEN protocols ospf export vrf-exp-ospf-def
    ...
    #R1 leak the Internet table(only bgp part in this case, so under BGP) to vrf
    delete logical-systems r1 routing-options rib-groups inet.0-to-vrf
    delete logical-systems r1 protocols bgp group to-rr family inet unicast rib-group inet.0-to-vrf
    delete logical-systems r1 protocols bgp group to-c3 family inet unicast rib-group 
    
#### solution2 config
    
    #config R1 static default to R2/3, then copy to vrf
...
### config: VPN InetAccess 3 (t.b.c)

    set logical-systems r1 routing-options aggregate route 20.20.0.0/16
...
#### remove sol3
    ...
    delete logical-systems r1 routing-instances GREEN routing-options static  
    delete logical-systems r1 policy-options policy-statement vrf-exp term 3 
...
### verify cos
    ...
    run show class-of-service classifier type inet-precedence
...
##### classifier: ingress traffic ==> Q (forwarding class)
    ...
    run show class-of-service classifier
...
R1:
    ...
    #config firewall policers:5m,7m,10m
    set logical-systems r1 firewall policer 5m if-exceeding bandwidth-limit 5m    
    set logical-systems r1 firewall policer 5m if-exceeding burst-size-limit 15k   
    set logical-systems r1 firewall policer 5m then discard                         
...
#### TIP: `ipv6-tunneling` vs `rib-group`

R1 don't need extra config to copy inet.3 into inet6.3, 
previous config: `ipv6-tunneling` under mpls will do it
while this magic is true for r1, which built inet.3 via rsvp/ldp; 
this doesn't work for other routers like R2/3/7/8. 
the way they build their inet.3 is through BGP (`inet labeled-unicast rib inet.3`)

如果inet.3中的路由是从ldp或者rsvp学来的，那么配置了ipv-tunneling之后，
这些路由会直接copy到inet6.3中，
如果路由是从bgp学来的，默认不会copy，需要做rib-group
...
because both routes (by default), will be put in inet.0 -- the only diff
between these 2 types of routes are, the labeled route carries a label -- this
will lead confliction so only one address family can't be supported
...
because labeled-route are now put in inet.3, unlabeled are in inet.0

now "You can allow both labeled and unlabeled routes to be exchanged in a single
session"
...
#### QUESTION(resolved):how C routers got the IPv6 routes?
...
##### config
    ...
    #R5/6: advertise R2/3/7/8 lo0 via BGP:
